...
void apresentacao();      // Apresenta a Animação e o Nome.
...
void sleep();             // Função implicita .
int menu_categoria();     // Exibe um menu para a seleção das categorias de palavras.
void initialize(int a);   // Cria a string de testes utilizada no jogo.
void display();           // Imprime a string de testes.
int entrada();            // Lê a letra fornecida pelo usuário.
...
void viveu();             // Imagem para caso de vitória.
void morreu();            // Animação para caso de derrota.
void alfabeto();          // Imprime string com letras do alfabeto.
void reset_valores();     // Reseta os valores das variáveis para que se possa jogar novamente.
...
int erros=0;
bool on_0 = 1, on_1 = 1, on_2 = 1, on_3 = 1;// Variáveis booleanas
...
// Começo das Funções:
//______________________________________________________________________________
// Faz a Barrinha de Carregando (sistematicamente inútil, porém esteticamente razoavel...)
...
void loading()
{
     ...
     printf("\n\t\t\t       Loading...");
     std::this_thread::sleep_for(std::chrono::milliseconds(200));
     system("cstd::this_thread::sleep_for(std::chrono::milliseconds(200));ls");
     printf("\n\n\n\n\t\t\t      [###_______] \n");
     printf("\n\t\t\t       Loading...");
     std::this_thread::sleep_for(std::chrono::milliseconds(200));
     system("cls");
...
/* Observe que para exibição de um printf de mais de uma linha, as aspas podem
ser utilizadas da maneira abaixo.

   Obs.: IMPORTANTE !! Se for fazer uma exibição de nome como a que tem abaixo
   não se esqueça que a contra-barra "\" no compilador é interpretada como um comando.
   para que ela apareça no seu printf voce deve usar o comando "\\".

   Obs2.: Não Tão Importante !!
...
printf("\n\n\t\t //)))))    __      ___       __      __                      \n"
           ...
           "\t\t ||       |    |   || \\\\    |       ||==||                  \n"
           "\t\t ||       \\\\__//   ||  \\\\   \\\\__//  ||  ||            \n\n"
           "\t\t                 altered by devlab 22/11/2023                 \n"
           "\t\t___________________________________________            \n\n\n");
}

//______________________________________________________________________________
// Pergunta se Quer Sair do Jogo
// Para mais explicações sobre os menus do jogo veja a função Menu()
...
//______________________________________________________________________________
// Exibe Animação e Nome Enfeitado
// Pra fazer uma animação dessa.. só quem num tem o que fazer mesmo.
...
//
//Exibe o Menu em que se escolhe as categorias das palavras que serão sorteadas
...
//______________________________________________________________________________
// Ativa a Mudança de Cor Pelo Pressionamento das Teclas
// Para mais explicações sobre os menus do jogo veja a função Menu()

/* Essa foi particularmente dificil de entender pra mim, ja que peguei a ídeia
dessa função de um outro jogo que me serviu de inspiração para começar esse projeto.

Para a função de troca de cor, voce deve entender duas coisas essenciais.
* A troca de Cores se faz num matriz onde o termo [6] modifica a cor do fundo
e o termo [7] modifica a cor das letras.
* O registro das Cores está em caracteres especificos que devem ser colocados
nos lugares certos dentro da matriz "cor".

Algumas Cores e Suas representações em caracteres:
'F' = Preto
'E' = branco
'D' = rosa
'C' = vermelho
...
usando o system("cls"); e a chamada de função system(cor);. Caso contrario
somente as proximos caracteres adicionados ganharão a nova cor. (isso, dependendo
...
/* Perceba que tantos printf's poderiam ter sido evitados se eu usasse uma função
...
/* Como ler caracteres sem Enter (de Batalha Naval 3.17 por H.N. Softwares.)
         ...
         Essa função deve ser definida quando se espera que o usuário digite uma
   tecla e faz com que ela seja lida como um numero do codigo ASCII.
         Como funciona:
                       O while(1) é definido para gerar um loop infinito, e nesse caso,
              serve para repetir a função até que seja reconhecido o pressionamento
              de uma tecla qualquer. O comando kbhit(), ou traduzindo: Key Board Hit,
              é o meio utilizado para continuar apenas se uma tecla for pressionada.
                       Logo em Seguida, a variavel "vartecla" é atribuida ao inteiro
              que representa a tecla.( Para descobrir o valor de uma tecla adicione
              ao final da função: printf("%d", vartecla); ). Como o retorno da função
              é a variável "vartecla", para chamar a função mais tarde, basta atribui-la
              á uma variável qualquer. (ex.: c = tecla() )
...
//______________________________________________________________________________
// Menu Principal
// O Menu é impresso e pode se usar as teclas para selecionar a opção escolhida


/* Uma Rápida explicação dos Menus do Jogo:
              Em todo o jogo, os menus são representados de maneiras possivelmente
       confusa. Para esclarecer a aqueles que não compreenderam bem o funcionamento,
       segue uma explicação:
             1) O Comando utilizado para capturar o código das teclas pressionadas
                é o Tecla(). Quando chama a função c = tecla(); então a variável
                c assumirá o valor no ASCII da Tecla Pressionada.
                  (ex.: baixo = 80; a = 97; [Enter] = 13)
             2) A variável "lugar" é atribuida à opção selecionada no momento
                no menu em que foi definida.
             3) Cada vez que uma tecla é pressionada o "if" é chamado:
                     if (kbhit()) {     Se uma tecla for pressionada faça...
             4) Dependendo do código atribuido a "c" um dos comandos de posição será
                ativado: j++, j--, ou se o usuario pressionar enter, a variável
                lugar será lida e colocada em um switch case, e dependendo do
                seu valor, diversas a tribuições podem ser feitas a ela.
...
-------------------------------------
FUNÇÕES RELATIVAS AO JOGO EM SI:
-------------------------------------*/
...
//______________________________________________________________________________
//Lê o palpite, com o cuidado de descartar entradas inválidas.
...
//Se errado, adiciona um à variável erros*/
...
//______________________________________________________________________________
//Animação para o caso de derrota.
void morreu()
{
     system("cls");
     printf ("_________   \n"
...
//______________________________________________________________________________
//Mostra o alfabeto e retira as letras já utilizadas, substituindo-as por '-'.
void alfabeto (void)
{
		int i;
		printf("\n");
...
Outros Comentários:
       ...
       Para esclarecer a divisão de tarefa dos autores do jogo:

            Italo Ayres foi responsável pelos Menus do jogo e todas a funções
                        referentes a eles, como: mudança de cor, captura de
                        tecla, animações e efeitos tridimensionais.

           João Martins idealizou e contruiu tudo que se refere ao jogo em si,
                        onde inclui-se o sistema de letras, vidas, categorias,
                        ramdomização das palavras e o extenso database do jogo.

     The One More Original Title trata-se na verdade de uma empresa totalmente
     fictícia (noossa que surpresa), porem todos os direitos de copyright estão
     reservados.. (anh?).


     "Faltou só o bye bye para finalizar"
                ( Ulysses sobre o jogo forca )

     "Na União Sovietica a Forca mata VOCE"
                ( Reversal Russa sem sentido sobre a Forca)

     "Esse tipo de jogo não tem a menor graça..."
                ( Tiradentes sobre Forca )

     "Como assim minha placa de video não suporta aceleração 3-D ?"
                ( Iury Rogério indignado com seu Sony Vaio Core i7 )
*/
